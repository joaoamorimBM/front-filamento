...
The `home-routes` handler in the `front-filamentos.routes.home` namespace
...
The `home-page` function will in turn call the `front-filamentos.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `front-filamentos.handler` namespace:
...
Request middleware functions are located under the `front-filamentos.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `front-filamentos.dev-middleware` namespace found in
